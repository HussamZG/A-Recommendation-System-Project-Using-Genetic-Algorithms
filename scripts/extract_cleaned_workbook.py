# سكربت استخراج البيانات من ملف Excel: يقرأ data_cleaned.xlsx ويُنتج ملفات JSON (users/products/interactions/summary)
...
# دالة حساب درجة اللياقة: شراء×5 + نقر×2 + مشاهدة×1 + تقييم×3 (نفس الأوزان المستخدمة في الخوارزمية الجينية)
def fitness_score(viewed: int, clicked: int, purchased: int, rating: int) -> int:
    return (purchased * 5) + (clicked * 2) + (viewed * 1) + (rating * 3)


# الدالة الرئيسية: قراءة أوراق العمل الأربعة، بناء الكائنات، حساب الإحصائيات، وكتابة ملفات JSON
def main() -> None:
    ...
    # بناء قائمة المستخدمين من ورقة users_clean
    users = [
        {
    ...
    # بناء خريطة التقييمات حسب (مستخدم، منتج) وحسب المنتج لحساب المتوسط لاحقاً
    ...
    # تجميع سلوك المستخدمين لكل منتج وحساب درجة اللياقة لكل تفاعل
    ...
    # بناء قائمة المنتجات مع متوسط التقييم ومقاييس السلوك
    products = []
    ...
    # ترتيب البيانات حسب المعرّف لضمان الاتساق
    ...
    # بناء ملخص إحصائي شامل: أسعار، مسار التحويل، توزيع الفئات والتقييمات، أفضل 5 منتجات
    summary = {
    ...
    # كتابة ملفات JSON النهائية إلى مجلد data/
